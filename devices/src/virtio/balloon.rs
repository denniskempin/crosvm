...
async fn handle_stats_queue(
    ...
    mut queue: Queue,
    queue_event: EventFd,
    mut stats_read_evt: EventAsync<EventFd>,
...
async fn handle_irq_resample(interrupt: Rc<RefCell<Interrupt>>) {
    let resample_evt = interrupt
        ...
        .try_clone()
        .unwrap();
    let mut resample_evt = EventAsync::new(resample_evt).unwrap();
    loop {
...
async fn wait_kill(kill_evt: EventFd) {
    let mut kill_evt = EventAsync::new(kill_evt).unwrap();
...
fn setup_stat_signal() -> std::result::Result<(EventFd, EventAsync<EventFd>), StatSignalError> {
