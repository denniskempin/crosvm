...
async fn flush_timer(disk_state: Rc<RefCell<DiskState>>) -> result::Result<(), OtherError> {
    ...
    let mut async_timer = EventAsync::new(timer).map_err(OtherError::AsyncTimerCreate)?;
...
async fn handle_irq_resample(interrupt: Rc<RefCell<Interrupt>>) -> result::Result<(), OtherError> {
    ...
    let mut resample_evt =
        EventAsync::new(resample_evt).map_err(OtherError::AsyncResampleCreate)?;
    loop {
        let _ = resample_evt
            .next_val()
            .await
...
async fn wait_kill(kill_evt: EventFd) -> result::Result<(), OtherError> {
    let mut kill_evt = EventAsync::new(kill_evt).map_err(OtherError::AsyncKillCreate)?;
